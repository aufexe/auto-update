...
local update_url = 'config/settings.ini' -- Путь к ini файлу. Позже нам понадобиться.
local update_path = getWorkingDirectory() .. "/update.ini"

local script_url = 'https://raw.githubusercontent.com/aufexe/auto-update/main/helper.lua' -- Путь скрипту.
...
function main()
    while not isSampAvailable() do wait(0) end
        ...
        if update_found then -- Если найдено обновление, регистрируем команду /update.
            sampRegisterChatCommand('update', cmd_update)  -- Если пользователь напишет команду, начнётся обновление.
...
function imgui.OnDrawFrame()
    local CorX, CorY, CorZ = getCharCoordinates(PLAYER_PED)
    imgui.SetNextWindowPos(imgui.ImVec2(x / 2, y / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(500, 300), imgui.Cond.FirstUseEver)
    imgui.Begin('Helper', main_window_state)
        imgui.Text('Данька твои корды: X:'..CorX..' Y:'..CorY..' Z:'..CorZ)
    imgui.End()
end
